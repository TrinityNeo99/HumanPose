    ...
    def run(self):
        ...
        for f in self.files:
            ...
            hrnet_api.generate_kps(os.path.join(self.data_dir, f), args,
                                   box_model,
                                   pose_model, pose_transform, save_key_points_csv=True, save_video=False,
                                   save_org_video_image=True)
        end = time.time()
...
if __name__ == '__main__':
    test = Extract(video_dir=r"/root/2DHPE/data/pingpong-109-coco-mini")
    test.run()
